...
export interface Toast {
  ...
  closable: boolean;
  action?: ToastAction;
  /** Internal — when first shown (for progress bar math). */
  createdAt: number;
 ...
 *
 * Hover-pause is delegated back to `pause()` / `resume()` so the timer
 * lives on the source of truth, not just visually.
 */
